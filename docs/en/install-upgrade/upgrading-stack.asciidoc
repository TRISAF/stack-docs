...
recent 5.6 before upgrading to {version}. {xpack} 5.6 (be more precise indicating
the fix version because for example the 5.6.15 does not contain the upgrade assistant)
...
(the update is done but after a restart kibana I do not like the new .kibana converted,
you still need to use the fix that can be found at:
https://discuss.elastic.co/t/kibana-unaccessible-after-using-migration-ui/101763
1 - Download the fix:
http://download.elastic.co/downloads/kibana/6-0-post-migration-fix.sh
You can download directly to your workstation or server with curl, for example:
